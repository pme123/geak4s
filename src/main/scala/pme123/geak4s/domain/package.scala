 ...
 *
 * This package contains all domain objects for the GEAK (Gebäudeenergieausweis der Kantone) system.
 *
 * Package Structure:
 * - project: Project information (Project, Client, Expert, BuildingLocation, BuildingData, EgidEdidGroup)
 * - building: Building usage (BuildingUsage)
 * - envelope: Building envelope (RoofCeiling, Wall, WindowDoor, Floor, ThermalBridge)
 * - hvac: HVAC systems (HeatProducer, HeatStorage, HeatingDistribution, HotWaterDistribution, Ventilation)
...
package object domain:
  ...
  object GeakProject:
    ...
    lazy val example: GeakProject = GeakProject(
      ...
      heatProducers = List(HeatProducer.example, HeatProducer.exampleHeatPump, HeatProducer.exampleSolar),
